...
void WordTree::Add(const string newWord)
{
    AddSeparate(newWord, root);
}

//AddSeparate: Private recursive function of add.
//Takes in a string and finds the correct spot in the tree to add it. 
void WordTree::AddSeparate(string newWord, WordNode* current)
{
    ...
    else
    {
        ...
        if (newWord.compare(current->word) == 0)//If the words are equal
        {
            current->occurences++;//increment the word count
        }
        else if (newWord.compare(current->word) < 0)//if word is higher up alphabetically
        {
            ...
            AddSeparate(newWord, current->left); //go to left node and repeat process
        }
        else if (newWord.compare(current->word) > 0)//if word is lower alphabetically
        {
            ...
            AddSeparate(newWord, current->right); //go to left node and repeat process
        }
...
void WordTree::OutputHelper( const WordNode * current)
{
    ...
    cout << current->word << " " <<  current->occurences<< endl;//Puts a space in between word and number
    if (current->right != NULL)
    {
